export const buildTree = (process) => {
    ...
    process.Tasks.forEach((task, index) => {
        _tree.nodes[task.Id] = {id: task.Id, name: task.Name, weight: undefined, rowNumber: undefined, index: index, dependencies: {count: 0, nodes: []}}
    })
    ...
    Object.values(_tree.nodes).forEach((item) => {
        if (_tree.colCount < item.weight) { _tree.colCount = item.weight }
        if (_tree.rowCount < item.rowNumber) { _tree.rowCount = item.rowNumber }
