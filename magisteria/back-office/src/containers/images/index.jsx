...
import { fetchingSelector, imagesSelector, searchResultSelector, currentSelector, getImages, addImage, saveImage, deleteImage, clearImages, searchImage, setCurrentImage, } from '#src/ducks/images';
...
let imagesCount = 0;
const Images = ({ lessons, images, permissions, fetching, actions, searchResult, currentImage, }) => {
    ...
    const lampColor = useMemo(() => (images && (images.length > 0) && images.some((item) => !item.isModerated) ? 'red' : 'green'), [images]);
    return (<div className="images-page form _scrollable-y">
      ...
      <div className="images-page__grid-container">
        ...
        {visibleImage && (<ModalContainer renderContent={() => <ImageView image={visibleImage} onClose={handleCloseImageView}/>}/>)}
        {currentImage
            && (<ModalContainer renderContent={(props) => (<ImageEditor image={currentImage} accessLevel={accessLevel} 
                // accessLevel={AccessLevels.images.edit}
                onClose={handleCloseEditor} onForward={handleForward} onBackward={handleBackward} onApply={handleApply} onAdd={openSearch} {...props}/>)}/>)}
        {searchVisible && (<ModalContainer renderContent={(props) => (<ImageSearch images={searchResult} onApply={handleApplySearch} onSearch={handleSearch} onClose={handleCloseSearchForm} onUpload={handleUploadImage} {...props}/>)}/>)}
      </div>
    </div>);
};
