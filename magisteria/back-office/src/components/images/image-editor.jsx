...
const Toolbar = styled.div `
  ...
  padding-bottom: 8px;
`;
export const ImageEditor = ({ image, accessLevel, onAdd, onClose, onApply, onForward, onBackward, }) => {
    const { control, handleSubmit, reset, } = useForm({ defaultValues: image });
    const { isDirty } = useFormState({ control });
    const [confirmCallback, setConfirmCallback] = useState(null);
    const formRef = useRef(null);
    const closeOnConfirm = useCallback((confirm) => {
        setConfirmCallback(null);
        if (confirm && onClose)
            onClose();
    }, [onClose, confirmCallback]);
    const backOnConfirm = useCallback((confirm) => {
        setConfirmCallback(null);
        if (confirm && onBackward)
            onBackward();
    }, [onBackward, confirmCallback]);
    const forwardOnConfirm = useCallback((confirm) => {
        setConfirmCallback(null);
        if (confirm && onForward)
            onForward();
    ...
    return (<form ref={formRef} className="image-editor" onSubmit={handleSubmit(onSubmit)}>
      <Toolbar>
      ...
      <div className="image-editor__data">
        <div className="image-editor__img">
          <img src={fileName} alt={image.description || ''}/>
        </div>
        <div className="image-editor__pane _with-custom-scroll">
          <div className="image-editor__pane-wrapper">
            <Controller name="isModerated" control={control} render={({ field }) => <Checkbox label="Прошло модерацию" disabled={accessLevel < AccessLevels.images.moderate} {...field}/>}/>
            <Controller name="name" control={control} render={({ field }) => <TextField label="Название" disabled={readOnly} multiline {...field}/>}/>
            <Controller name="description" control={control} render={({ field }) => <TextField label="Описание" disabled={readOnly} multiline {...field}/>}/>
            <Controller name="altAttribute" control={control} render={({ field }) => <TextField label="Доп.атрибуты" disabled={readOnly} multiline {...field}/>}/>
            <Controller name="artifactText" control={control} render={({ field }) => <TextField label="Артефакты" disabled={readOnly} multiline {...field}/>}/>
            <Controller name="authorText" control={control} render={({ field }) => <TextField label="Автор" disabled={readOnly} multiline {...field}/>}/>
            <Controller name="museumText" control={control} render={({ field }) => <TextField label="Музей" disabled={readOnly} multiline {...field}/>}/>
            <Controller name="descriptor" control={control} render={({ field }) => <TextField label="Дескриптор" disabled={readOnly} multiline {...field}/>}/>
            <Controller name="showInGallery" control={control} render={({ field }) => <Checkbox label="Показывать в галереи" disabled={readOnly} {...field}/>}/>
            <Controller name="isFragment" control={control} render={({ field }) => <Checkbox label="Фрагмент" disabled={readOnly} {...field}/>}/>
            <Controller name="linkTypeId" control={control} render={({ field }) => (<RadioButtonsGroup label="Тип связи" disabled={readOnly} options={[
