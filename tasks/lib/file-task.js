...
exports.FileTask = class FileTask extends Task {
    ...
    _saveFile(data, startTime, finTime, fileName, fileGuid) {
        let checkSum;
        let lastMdf;
        return this._getFileInfo(fileName, fileGuid)
            .then((fileInfo) => {
                let content = data;
                let csContent = data;
                if ((typeof (data) !== "string") && data.content && data.csContent){
                ...
                if (isModified)
                    rc = rc
                        .then(() => {
                            ...
                            return writeFileAsync(fileName, content + footer);
                        })
                        .then(() => { return true; });
                return rc;
            });
        ...
        return new Promise((resolve, reject) => {
            ...
            .then((data) => {
                if (data) {
                    ...
                    return this._saveFile(data, startTime, new Date(), fileName, fileGuid);
                }
                else
                    return false;
            });
