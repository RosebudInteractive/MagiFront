                ...
                return new Promise((resolve, reject) => {
                    ...
                    if (!options.lock) {
                        ...
                        request(reqUrl.href, (error, response, body) => {
                            if (error) {
                                console.error(buildLogString(`FB Counter Error: "${error.message}",` +
                                ...
                                if (response.statusCode === HttpCode.OK) {
                                    ...
                                    if (usageStat) {
                                        try {
                                            ...
                                            if ((usageStat.call_count > options.usageLimitPerc)
                                                ...
                                                console.error(buildLogString(`SnCounterTask: FB app usage limits have exceeded a threshold (${options.usageLimitPerc}%) : ` +
                                    ...
                                    if (response.statusCode === HttpCode.ERR_FORBIDDEN) {
                                        options.lock = true;
                                        console.error(buildLogString(`SnCounterTask: FB access error: ${body}`));
                                    }
                                }
                                resolve(counter);
                            }
    ...
    */
    _getVKCount(url, options) {
        ...
        return getCounter(url)
            ...
            .catch(err => {
                console.error(buildLogString(`VK Counter Error: "${err.message}",` +
    ...
    */
    _getOKCount(url, options) {
        ...
        return getCounter(url)
            ...
            .catch(err => {
                console.error(buildLogString(`OK Counter Error: "${err.message}",` +
                    ` API: "${reqUrl ? reqUrl.href : "???"}", URL: "${url}"`));
                return 0;
            });
