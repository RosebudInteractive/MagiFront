    ...
    return new Promise((resolve, reject) => {
        ...
        .then((template) => {
            let body = _.template(template)(
                {
                    link: config.proxyServer.siteHost + config.authentication.activationRoute + '/' + data.ActivationKey
                });
            let mailOptions = {
            ...
            return SendMail(MAIL_CFG_NAME, mailOptions)
                .then((mailResult) => {
                    let _obj = userCache.userToClientJSON(user);
                    if (mailResult && mailResult.msgUrl) {
                        _obj.msgUrl = mailResult.msgUrl
                    }
