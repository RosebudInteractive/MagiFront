...
function sendRequest(path, method, data, useToken, callback){
    ...
    let errHandler = (err, isDebug) => {
        let msg = `sendpulse::sendRequest ERROR Info:${(err && err.code) ? (" code: " + err.code) : ""}` +
            `${(err && err.message) ? (" message: " + err.message) : ""}` +
            ` host: ${options.hostname} path: ${options.path} method: ${options.method}`;
        console.error(buildLogString(msg));
        if (callback) {
            let data = returnError(msg);
            if (isDebug)
    ...
    // errHandler({ code: "TESTCODE", message: "Test Message" }, true);
