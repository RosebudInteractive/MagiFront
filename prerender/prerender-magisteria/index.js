//
// This is a patched version of a "prerender-node" package "3.2.0" ("https://github.com/prerender/prerender-node#readme")
//
//
...
prerender.gunzipResponse = function(response, callback) {
  ...
  gunzip.on('data', function (chunk) {
    //
    // Alexander Sokolov: It's not correct to concatenate a Buffer to a string, because
    //   the Buffer may not content entire symbol at the end. Fixed.
    //  
    dataChanks.push(chunk);
    // content += chunk;
  });
  gunzip.on('end', function() {
    response.body = Buffer.concat(dataChanks).toString();
