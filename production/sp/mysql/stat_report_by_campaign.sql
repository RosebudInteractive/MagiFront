CREATE DEFINER=`sa`@`%` PROCEDURE `stat_report_by_campaign`(st_date datetime, fin_date datetime)
BEGIN
select c.id, st_date, fin_date, coalesce(concat(c.Campaign, ' (', c.Source, '+', c.Medium, ')'), '<Empty>') campaign,
  res.nreg, res.hours, res.hnew, res.nlsn, res.nmain, res.nuser, res.nfull, res.sale_qty, res.sale_sum, res.sale_sum_ios
from
(
  ...
  union all
  -- кол-во регистраций
    select coalesce(u.CampaignId, 0), count(*), 0, 0, 0, 0, 0, 0, 0, 0, 0 from User u
  ...
  -- сколько часов прослушали "новые" пользвоатели (зарегистрированы меньше Х дней, пока ставим Х=5)
    select coalesce(h.CampaignId, 0), 0, 0, round(sum(LsnTime)/60/60,2), 0, 0, 0, 0, 0, 0, 0 from LsnHistory h
